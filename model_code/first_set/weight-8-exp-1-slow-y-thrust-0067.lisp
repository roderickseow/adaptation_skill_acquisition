...
; - Slowdown
;   - Yes
; - Controller min-thrust-time: 0.067
; - Max-comfort-speed
;   - Yes: 3.0
...
(defparameter *min-thrust-time* 0.067)
(defparameter *min-press-point* 0.0)


(defparameter *bad-weight* -8)

(defparameter *decayMethod* 'exponential)
(defparameter *decay* 1)


(defparameter *slowdown* "\"slowdown\"")
...
;productions for speed control in spacetrack
(p no-adjustment
   =goal>
      state do-step
      ship-speed =speed
...
!safe-eval! (>= =val (+ =speed 1))
     ==>
!safe-bind! =angle (mod (- =current 180) 360)
   =goal>
      adjusted t
      nextstep tapthrust
      state turn-to-point
